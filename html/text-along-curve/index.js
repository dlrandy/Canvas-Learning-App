...
const textOnCurve = (ctx, text, offset, x1, y1, x2, y2, x3, y3, x4, y4) => {
  ctx.save();
  ...
  let ch = curveHelper(x1, y1, x2, y2, x3, y3, x4, y4);
  var pos = offset;
  var cpos = 0;
...
const curveHelper = (x1, y1, x2, y2, x3, y3, x4, y4) => {
  let tx1, ty1, tx2, ty2, tx3, ty3, tx4, ty4;
  let a, b, c, u;
  let vec, currentPos, vec1, vect;
  vec = { x: 0, y: 0 };
  vec1 = { x: 0, y: 0 };
  vect = { x: 0, y: 0 };
  quad = false;
  ...
  function forward(dist) {
    ...
    while (currentDist < dist) {
      ...
      currentPos += onePix;
      helper.posAt(currentPos);
      currentDist += step = Math.sqrt(
        (vec.x - vec1.x) * (vec.x - vec1.x) +
          (vec.y - vec1.y) * (vec.y - vec1.y)
      );
    }
...
textOnCurve(
  ctx,
  "Hello world! dadad  dadad adad adada dada dad da a",
  50,
  100,
  100,
  200,
...
// textOnCurve(ctx, "Hello world!", 50, 100, 100, 200, 200, 300, 100, 400, 200);
